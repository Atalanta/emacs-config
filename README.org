...
Some important environment variables like PATH are not inherited by Emacs when launched from the GUI on macOS. This package grabs those from the shell so Emacs can find tools like Git, Ruby, etc. On Linux this is less critical but harmless.
...
Set zsh as the default shell for Emacs shell buffers and vterm, but fall back to bash if zsh is not available. This makes the config portable across macOS and Linux systems.
...
Use JetBrains Mono, a modern programming font with excellent readability and ligature support. Falls back to monospace if not installed.
